...
服务端是**权威**的：开火、碰撞、拾取都以服务器上的坦克位置为准。客户端为了跟手，会做**预测 + 输入回放**：

1. 你按键 → 客户端立刻本地移动（预测），同时把输入（带 `seq` 序号）发给服务器
2. 服务器每 tick 仿真，并回传 **`lastInputSeq`**（已处理到第几号输入）和坦克位置
3. 客户端收到快照时：**只在 `lastInputSeq` 变化时**才用服务器位置做基准，**回放**尚未确认的输入（`seq > lastInputSeq`）
4. 帧与帧之间继续用当前按键做预测，保证画面实时

这样移动仍然跟手，开火位置也与服务器一致，且不会因为「每帧硬拉回服务器」而卡顿。

::: tip 仍觉得卡？
多为 **网络延迟** 或 **Fly 单核** 上房间过多。见下方「会不会卡？」；静止时不应再出现反复微抖（v0.3.16 已修）。
:::
