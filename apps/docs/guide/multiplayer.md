...
服务端是**权威**的：开火、碰撞、拾取都以服务器上的坦克位置为准。客户端在 `apps/client/src/net/onlinePrediction.ts` 里做三层分工：

| 层 | 作用 |
|----|------|
| **60Hz 统一 tick** | 发输入与本地预测共用同一时钟，与服务端 `BattleRoom` 一致 |
| **输入回放** | `lastInputSeq` 变化时，以服务器位置为基准回放未确认输入 |
| **tick 内插值** | 高帧率下在相邻 tick 姿态之间插值渲染，画面顺滑 |

流程简述：

1. 每 **1/60 秒**发送一条 `input`（带 `seq`），并推进本地预测一步
2. 服务器仿真后回传 **`lastInputSeq`** 与坦克位置
3. 仅在 **`lastInputSeq` 变化**或偏差过大时做完整回放（避免子弹/分数等无关补丁拉扯坦克）
4. 渲染使用 **插值后的 display pose**，预测层仍与服务器对齐

这样兼顾 **跟手**、**不卡**、**开火/拾取与服务器一致**。

::: tip 仍觉得卡？
多为 **网络延迟** 或 **Fly 单核** 上房间过多。见下方「会不会卡？」。
:::
